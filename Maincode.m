...
%         end
    ...
    radius(iter +1)    = sqrt((coord(loadx,1)+a(loadx))^2+(coord(loadx,2)+a(loady))^2)/R0;
...
plot(loadpercentvec(index),radius(index));
title('Normalised radius against loadpercent')
xlabel('Percent of initial force')
ylabel('Normalised radius r/R0')
